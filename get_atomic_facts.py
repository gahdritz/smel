...
class LocalLlama(LM):
    ...
    def _generate_chat(self, prompts, responses, max_sequence_length=2048, max_output_length=128):
        ...
        assert(len(prompts) == len(responses) + 1)
...
class AtomicFactGenerator(object):
    ...
    def get_init_atomic_facts_from_sentence(self, sentences, cost_estimate=None):
        ...
        atomic_prompt = "Please break the following sentence down into independent, unique facts:"
        ...
        for sentence in sentences:
            ...
            for i in range(n):
                prompt = f"{atomic_prompt} {list(demons.keys())[i]}"
            ...
            prompt = f"You may use information from the rest of the paragraph as context: {'. '.join(sentences)}.\n\n{atomic_prompt} {sentence}"
            prompts.append(prompt)

            all_prompts.append((prompts, responses))
        ...
        if cost_estimate:
            ...
            for i, (prompts, responses) in enumerate(all_prompts):
                output, _ = self.lm.generate_chat(prompts, responses)

                # Light postprocessing
                output = output.split("\n \n")[0] # prevents "\n \nAlternatively, ..."

                atoms[sentences[i]] = text_to_sentences(output)
...
def main():
    ...
    def is_bio(text):
        ...
        dates = first_sentence.split('(')[-1].split(')')[0].lower()
        months = ["january", "february", "march", "april", "may", "june", "july", "august", "september", "october", "november", "december"]
        if(not any([m in dates for m in months])):
            return False

        return True

    db_path = os.path.join(DATA_DIR, f"{DB_NAME}.db")
    db = DocDB(db_path=db_path, data_path=None)

    texts = db.get_all_text()
    random.shuffle(texts) 
    print("DB loaded!")

    lm = LocalLlama("meta-llama/Llama-3.3-70B-Instruct")
    generator = AtomicFactGenerator(lm, "demos")
    ...
    for i, t in enumerate(texts):
        ...
        t = t[0].split(SPECIAL_SEPARATOR)[0]
        #t = t.replace("<s>", "")
        #t = t.replace("</s>", "")
        t = t.rsplit('.', 1)[0]

        atomic_facts, para_breaks = generator.run(t)

        outputs.append(atomic_facts)

        if(len(outputs) % 100 == 0):
            import pickle
            with open("facts.pickle", "wb") as fp:
